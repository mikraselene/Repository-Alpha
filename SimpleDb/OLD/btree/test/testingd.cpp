...
TEST_F(DiskBasedBtree, TestC)
{
    std::shared_ptr<Pager> pm =
        std::make_shared<Pager>("b+tree.index", true);
    ...
    for (int i = 1; i < 100; i++)
    {
        Student p{i, true, "amaru", "orihuela", 32};
        page_id = i;
        record_manager.save(page_id, &p);
        bt.insert(Pair{p.key, page_id});
    }
    bt.print_tree();
    /*
...
{
    std::shared_ptr<Pager> pm =
        std::make_shared<Pager>("b+tree.index", false);
    btree<Pair, BTREE_ORDER> bt(pm);
    Pager record_manager("students.bin", false);
    int64_t page_id;
