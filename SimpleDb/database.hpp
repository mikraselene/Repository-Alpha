...
private:
    ...
    uint file_length_;
    pointer pages_[database::MAX_PAGES];
};
...
{
    memcpy(CAST(to) + database::KEY_OFFSET, &key_, database::KEY_SIZE);
    memcpy(CAST(to) + database::LPOS_OFFSET, &left_pos_, database::POS_SIZE);
    memcpy(CAST(to) + database::RPOS_OFFSET, &right_pos_, database::POS_SIZE);
}
void Row::deserialize_from(pointer from)
{
    memcpy(&key_, CAST(from) + database::KEY_OFFSET, database::KEY_SIZE);
    memcpy(&left_pos_, CAST(from) + database::LPOS_OFFSET, database::POS_SIZE);
    memcpy(&right_pos_, CAST(from) + database::RPOS_OFFSET, database::POS_SIZE);
}
void Row::print()
{
    printf("%s", key_);
...
{
    if (page_num > database::MAX_PAGES)
    {
        throw "dd";
    }
...
void Pager::flush(uint page_num, uint size) // TODO:
{
    if (pages_[page_num] == nullptr)
    {
        exit(1);
...
{
    uint page_num = row_num / database::ROWS_PER_PAGE;
    pointer page = pager_->get_page(page_num);
    uint offset = (row_num % database::ROWS_PER_PAGE) * database::ROW_SIZE;
...
{
    uint page_num = row_num_ / database::ROWS_PER_PAGE;
    pointer page = table_->pager()->get_page(page_num);
    uint offset = (row_num_ % database::ROWS_PER_PAGE) * database::ROW_SIZE;
    return CAST(page) + offset;
}
...
auto Database::dbname() -> string { return dbname_; }
#pragma endregion
