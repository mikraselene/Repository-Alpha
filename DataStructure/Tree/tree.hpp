#ifndef DATASTRUCTURE_TREE_TREE_HPP_
#define DATASTRUCTURE_TREE_TREE_HPP_
// -*- C++ -*-
//===------------------------------ tree ------------------------------===//
...
template <class T, class Allocator = std::allocator<tree_node_<T>>>
class tree {
 public:
  ...
  class iterator_base_less {
   public:
    bool operator()(
        const typename tree<T, Allocator>::iterator_base &one,
  ...
  tree<value_type, allocator_type> &operator=(
      const tree<value_type, allocator_type> &);  // FIXME:
  ...
  // 把给定区间 [begin, end] 内的结点 (包含它们的子树) 插入为给定结点 pos
  // 的终末子结点.
  template <class iterator_type>
  iterator_type append_children(iterator_type pos, sibling_iterator begin,
                                sibling_iterator end);
  // 把给定区间 [begin, end] 内的结点 (包含它们的子树) 插入为给定结点 pos
  // 的起始子结点.
  template <class iterator_type>
  iterator_type prepend_children(iterator_type pos, sibling_iterator begin,
                                 sibling_iterator end);
  ...
  //
  template <class iterator_type>
  iterator_type insert_subtree_after(iterator_type pos,
                                     const iterator_base &base);
  ...
  // 把给定区间 [pos_begin, pos_end] 内的结点替换为另一个区间 [other_begin,
  // other_end] 内的结点 (包含子树). TODO:
  sibling_iterator replace(sibling_iterator pos_begin, sibling_iterator pos_end,
                           sibling_iterator other_begin,
                           sibling_iterator other_end);
  ...
  template <class iterator_type>
  iterator_type re_parent(iterator_type pos, sibling_iterator begin,
                          sibling_iterator end);
  ...
  void sort(sibling_iterator begin, sibling_iterator end,
            StrictWeakOrdering cmp, bool deep = false);
  // WHY? TODO:
  template <class iterator_type>
  bool equal(const iterator_type &one, const iterator_type &two,
             const iterator_type &three) const;
  // WHY? TODO:
  template <class iterator_type, class BinaryPredicate>
  bool equal(const iterator_type &one, const iterator_type &two,
             const iterator_type &three, BinaryPredicate) const;
  // 检查两个结点的子树是否相等. TODO:
  template <class iterator_type>
  bool equal_subtree(const iterator_type &, const iterator_type &) const;
  // 检查两个结点的子树是否相等. TODO:
  template <class iterator_type, class BinaryPredicate>
  bool equal_subtree(const iterator_type &, const iterator_type &to,
  ...
  // 获得给定两结点的最近共同祖先.
  pre_order_iterator lowest_common_ancestor(const iterator_base &pos1,
                                            const iterator_base &pos2) const;
...
template <class value_type, class allocator_type>
value_type *tree<value_type, allocator_type>::iterator_base::operator->()
    const {
  return &(node->data);
}
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::iterator_base::begin() const {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::iterator_base::end() const {
...
template <class value_type, class allocator_type>
class tree<value_type, allocator_type>::pre_order_iterator
    : public tree<value_type, allocator_type>::iterator_base {
 public:
  pre_order_iterator();
  ...
  bool operator!=(const pre_order_iterator &other) const;
  bool is_last() { return this->node->next_sibling == NULL; }
  bool is_q() { return this->node->first_child == NULL; }
...
template <class value_type, class allocator_type>
tree<value_type, allocator_type>::pre_order_iterator::pre_order_iterator()
    : iterator_base() {}
...
tree<value_type, allocator_type>::pre_order_iterator::pre_order_iterator(
    const iterator_base &base)
    : iterator_base(base.node) {}

template <class value_type, class allocator_type>
tree<value_type, allocator_type>::pre_order_iterator::pre_order_iterator(
    const sibling_iterator &sib)
    : iterator_base(sib.node) {
  // WHY?
  if (this->node == NULL) {
    if (sib.last() != NULL) {
      this->node = sib.last();
...
template <class value_type, class allocator_type>
bool tree<value_type, allocator_type>::pre_order_iterator::operator==(
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator &
tree<value_type, allocator_type>::pre_order_iterator::operator++() {
...
template <class value_type, class allocator_type>
const typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::pre_order_iterator::operator++(int) {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator &
tree<value_type, allocator_type>::pre_order_iterator::next_skip_children() {
  (*this).skip_children();
  (*this)++;
  return *this;
}
...
template <class value_type, class allocator_type>
class tree<value_type, allocator_type>::sibling_iterator
    : public tree<value_type, allocator_type>::iterator_base {
 public:
  sibling_iterator();
...
template <class value_type, class allocator_type>
tree<value_type, allocator_type>::sibling_iterator::sibling_iterator()
    : iterator_base() {
  set_parent();
}
...
template <class value_type, class allocator_type>
bool tree<value_type, allocator_type>::sibling_iterator::operator==(
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator &
tree<value_type, allocator_type>::sibling_iterator::operator++() {
  if (this->node != NULL) {
    this->node = this->node->next_sibling;
  }
  return *this;
...
template <class value_type, class allocator_type>
const typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::sibling_iterator::operator++(int) {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator &
tree<value_type, allocator_type>::sibling_iterator::operator+=(int increment) {
  while (increment > 0) {
    (*this)++;
    increment--;
  }
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator &
tree<value_type, allocator_type>::sibling_iterator::operator-=(int decrement) {
  while (decrement > 0) {
    (*this)--;
    decrement--;
  }
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::tree_node *
tree<value_type, allocator_type>::sibling_iterator::first() const {
  return parent->first_child;
}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::tree_node *
tree<value_type, allocator_type>::sibling_iterator::last() const {
...
template <class value_type, class allocator_type>
class tree<value_type, allocator_type>::post_order_iterator
    : public tree<value_type, allocator_type>::iterator_base {};

template <class value_type, class allocator_type>
class tree<value_type, allocator_type>::level_order_iterator
    : public tree<value_type, allocator_type>::iterator_base {};

template <class value_type, class allocator_type>
class tree<value_type, allocator_type>::leaf_iterator
    : public tree<value_type, allocator_type>::iterator_base {};
...
//
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::print(pre_order_iterator begin,
  ...
  while (it != end) {
    ...
    for (int i = 1 + root_depth; i < current_depth; i++) {
      std::cout << COLOR
                << (find(overs.begin(), overs.end(), i) == overs.end() ? "│ "
                                                                       : "  ")
                << RESET;
    }
    if (current_depth > next_depth) {
      over_depth = -1;
      if (!overs.empty() &&
          *max_element(overs.begin(), overs.end()) > next_depth) {
        overs.clear();
      }
    }
    if (current_depth != root_depth) {
      std::cout << COLOR;
      std::cout << (flag ? "└─" : "├─");
      std::cout << (current_it.is_q() == 0 ? "┯" : "━");
      std::cout << (" ");
      std::cout << RESET;
    }
    std::cout << (*current_it);
    if (children_count(current_it) != 0) {
      std::cout << COLOR << "(" << children_count(current_it) << ")" << RESET;
    }
    std::cout << std::endl;
  }
  std::cout << "════════════════════════════════════════════════\n";
...
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::copy(
    const tree<value_type, allocator_type> &other) {
  clear();
...
template <class value_type, class allocator_type>
tree<value_type, allocator_type> &tree<value_type, allocator_type>::operator=(
    const tree<value_type, allocator_type> &other) {
  if (this != &other) {
    // FIXME: copy_(other);
  }
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::begin() const {
  // WHY?
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::end() const {
  return pre_order_iterator(feet);
}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::pre_begin() const {
  return begin();
}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::pre_end() const {
  return end();
}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::in_order_iterator
tree<value_type, allocator_type>::in_begin() const {}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::in_order_iterator
tree<value_type, allocator_type>::in_end() const {}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::post_order_iterator
tree<value_type, allocator_type>::post_begin() const {}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::post_order_iterator
tree<value_type, allocator_type>::post_end() const {}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::begin(const iterator_base &pos) const {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::end(const iterator_base &pos) const {
  // WHY?
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::prev_sibling(
    iterator_type pos) {
  assert(pos.node != NULL);
  iterator_type t(pos);
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::next_sibling(
    iterator_type pos) {
  assert(pos.node != NULL);
  iterator_type t(pos);
...
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::erase_children(
    const iterator_base &base) {
  // WHY?
  if (base.node == NULL) {
    return;
...
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::erase_right_siblings(
...
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::erase_left_siblings(
    const iterator_base &pos) {
  // WHY?
  if (pos.node == NULL) {
    return;
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::append_child(
    iterator_type pos) {
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::prepend_child(
    iterator_type pos) {
  assert(pos.node != head && pos.node != feet && pos.node != NULL);
  tree_node *n = alloc.allocate(1, NULL);
  alloc.construct(n, tree_node());
...
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::append_child(
    iterator_type pos, const value_type &val) {
  assert(pos.node != head && pos.node != feet && pos.node != NULL);
  tree_node *n = alloc.allocate(1, NULL);
...
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::append_child(
    iterator_type pos, iterator_type other) {
  assert(pos.node != head && pos.node != feet && pos.node != NULL);
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::append_children(
    iterator_type pos, sibling_iterator begin, sibling_iterator end) {
  assert(pos.node != head && pos.node != feet && pos.node != NULL);
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::prepend_children(
    iterator_type pos, sibling_iterator begin, sibling_iterator end) {
  assert(pos.node != head && pos.node != feet && pos.node != NULL);
  iterator_type t = begin;
  while (begin != end) {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::set_head(const value_type &val) {
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::insert(iterator_type pos,
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::insert(sibling_iterator pos,
                                         const value_type &val) {
  // WHY?
  tree_node *t = alloc.allocate(1, NULL);
  alloc.construct(t, val);
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::insert_subtree(
    iterator_type pos, const iterator_base &base) {
  // WHY?
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::insert_after(
    iterator_type pos, const value_type &val) {
  tree_node *t = alloc.allocate(1, NULL);
  alloc.construct(t, val);
  t->first_child = NULL;
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::insert_subtree_after(
    iterator_type pos, const iterator_base &base) {
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::replace(iterator_type pos,
                                                        const value_type &val) {
  pos.node->data = val;
  return pos;
}
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::replace(
    iterator_type pos, const iterator_base &other) {
  // WHY?
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::replace(sibling_iterator pos_begin,
                                          sibling_iterator pos_end,
                                          sibling_iterator other_begin,
                                          sibling_iterator other_end) {
  // FIXME:
}
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::re_parent(
    iterator_type pos, sibling_iterator begin, sibling_iterator end) {
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::re_parent(iterator_type pos,
                                                          iterator_type from) {
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::wrap(iterator_type pos,
                                                     const value_type &val) {
  // WHY?
  assert(pos.node != NULL);
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::move_after(iterator_type to,
                                                           iterator_type from) {
  // WHY?
  tree_node *node_to = to.node;
  tree_node *node_from = from.node;
  if (node_to == node_from ||
      (node_to->next_sibling != NULL && node_to->next_sibling == node_from)) {
    return from;
  }
  (node_from->prev_sibling != NULL ? node_from->prev_sibling->next_sibling
                                   : node_from->parent->first_child) =
      node_from->next_sibling;
  (node_from->next_sibling != NULL ? node_from->next_sibling->prev_sibling
                                   : node_from->parent->last_child) =
      node_from->prev_sibling;
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::move_before(
    iterator_type to, iterator_type from) {}

template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::move_before(sibling_iterator to,
                                              sibling_iterator from) {}

template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::move_on_top(
    iterator_type to, iterator_type from) {}

template <class value_type, class allocator_type>
tree<value_type, allocator_type> tree<value_type, allocator_type>::move_out(
    pre_order_iterator) {}
...
template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::move_in_below(iterator_type,
                                                              tree &) {}

template <class value_type, class allocator_type>
template <class iterator_type>
iterator_type tree<value_type, allocator_type>::move_in_as_nth_child(
    iterator_type, size_t, tree &) {}

template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::merge(sibling_iterator to1,
...
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::sort(sibling_iterator, sibling_iterator,
                                            bool deep) {}

template <class value_type, class allocator_type>
template <class StrictWeakOrdering>
void tree<value_type, allocator_type>::sort(sibling_iterator A,
                                            sibling_iterator B,
                                            StrictWeakOrdering cmp, bool deep) {
}

template <class value_type, class allocator_type>
template <class iterator_type>
bool tree<value_type, allocator_type>::equal(const iterator_type &one,
                                             const iterator_type &two,
                                             const iterator_type &three) const {
}
...
bool tree<value_type, allocator_type>::equal(const iterator_type &one,
                                             const iterator_type &two,
                                             const iterator_type &three,
                                             BinaryPredicate) const {}

template <class value_type, class allocator_type>
template <class iterator_type>
bool tree<value_type, allocator_type>::equal_subtree(
    const iterator_type &, const iterator_type &) const {}

template <class value_type, class allocator_type>
template <class iterator_type, class BinaryPredicate>
bool tree<value_type, allocator_type>::equal_subtree(const iterator_type &,
                                                     const iterator_type &to,
                                                     BinaryPredicate) const {}

template <class value_type, class allocator_type>
tree<value_type, allocator_type> tree<value_type, allocator_type>::subtree(
...
template <class value_type, class allocator_type>
void tree<value_type, allocator_type>::swap(pre_order_iterator this_it,
                                            pre_order_iterator that_it) {
...
template <class value_type, class allocator_type>
int tree<value_type, allocator_type>::max_depth(
...
template <class value_type, class allocator_type>
int tree<value_type, allocator_type>::children_count(
    const iterator_base &base) {
...
template <class value_type, class allocator_type>
int tree<value_type, allocator_type>::siblings_count(
    const iterator_base &base) const {
  tree_node *pos = base.node;
  int cnt = 0;
  while (pos->next_sibling != NULL && pos->next_sibling != head &&
         pos->next_sibling != feet) {
    cnt++;
    pos = pos->next_sibling;
  }
  pos = base.node;
  while (pos->prev_sibling != NULL && pos->prev_sibling != head &&
...
template <class value_type, class allocator_type>
bool tree<value_type, allocator_type>::is_in_subtree(
    const iterator_base &base, const iterator_base &begin,
    const iterator_base &end) const {
...
template <class value_type, class allocator_type>
bool tree<value_type, allocator_type>::is_valid(
    const iterator_base &base) const {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::pre_order_iterator
tree<value_type, allocator_type>::lowest_common_ancestor(
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::child(const iterator_base &base, int n) {
...
template <class value_type, class allocator_type>
typename tree<value_type, allocator_type>::sibling_iterator
tree<value_type, allocator_type>::sibling(const iterator_base &base, int n) {
  tree_node *t = base.node->parent == NULL ? head->next_sibling
                                           : base.node->parent->first_child;
  while (n--) {
    t = t->next_sibling;
  }
  return t;
...
#endif  // DATASTRUCTURE_TREE_TREE_HPP_
