...
void dfsgetfirst(int nv, int nump) //当前的符号，和相应产生式编号
{
    ...
    else
    {
        ...
        {
            if (proce[nump][0] == proce[nump][1])
                continue; //左递归的产生式不用不影响求fisrt集
...
bool analyze()
{
    ...
    for (int i = 0;;) //i，读入文本的
    {
        ...
        else if (tb_s_r[cur][getnum[word[i]]] == -2) //归约
        {
            ...
            cout << "进行归约,"
                 << "goto[" << cur << "," << getnum[word[i]] << "]=" << table[cur][getnum[proce[numpro][0]]];
            cout << "入栈" << endl;
